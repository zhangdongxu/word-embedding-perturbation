    ...
    for _wordk in ${wordkeep[@]}; do
        for _randt in ${randtype[@]}; do
            for _time in ${runtimes[@]}; do
                ...
                commands+=("srun --gres=gpu:1 --mem=$MEM --partition=${gpu_partition} ${CMD} $tune_args > log.sst2.${_randt}.${_fix}.${_wordk}.${_time}")
                #commands+=("srun --gres=gpu:1 --mem=$MEM --partition=${gpu_partition} ${CMD} $tune_args > log.mr.${_randt}.${_fix}.${_wordk}.${_time}")
                #commands+=("srun --gres=gpu:1 --mem=$MEM --partition=${gpu_partition} ${CMD} $tune_args > log.trec.${_randt}.${_fix}.${_wordk}.${_time}")
                #commands+=("srun --gres=gpu:1 --mem=$MEM --partition=${gpu_partition} ${CMD} $tune_args > log.cr.${_randt}.${_fix}.${_wordk}.${_time}")
            done
        done
    done
done
