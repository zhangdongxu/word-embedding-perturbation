    ...
    for _wordk in ${wordkeep[@]}; do
        for _randt in ${randtype[@]}; do
            for _time in ${runtimes[@]}; do
                tune_args="--train_emb=$_fix --word_keep=$_wordk --random_type=$_randt "
                commands+=("srun --gres=gpu:1 --mem=$MEM --partition=${gpu_partition} ${CMD} $tune_args > log.sst2.trainvocab.${_randt}.${_fix}.${_wordk}.${_time}")
                #commands+=("srun --gres=gpu:1 --mem=$MEM --partition=${gpu_partition} ${CMD} $tune_args > log.mr.trainvocab.${_randt}.${_fix}.${_wordk}.${_time}")
                #commands+=("srun --gres=gpu:1 --mem=$MEM --partition=${gpu_partition} ${CMD} $tune_args > log.trec.trainvocab.${_randt}.${_fix}.${_wordk}.${_time}")
                #commands+=("srun --gres=gpu:1 --mem=$MEM --partition=${gpu_partition} ${CMD} $tune_args > log.cr.trainvocab.${_randt}.${_fix}.${_wordk}.${_time}")
            done
        done
    done
done
